...
module CodeWars
  class Launcher
    ...
    def launch(event, from_decision = nil)
      ...
      unless event.available_decisions.count.zero? &&
      ...
      from_decision.made_at = Time.now if from_decision

      selected_decision, input_was_invalid = ask_player_to_make_decision(event)

      if !selected_decision && input_was_invalid
        puts end_game_label.colorize(:yellow)
        return
      end

      if event.requires_boss_beaten &&
         selected_decision.life_points && selected_decision.life_points.to_i.positive?
    ...
    def ask_player_to_make_decision(event)
      ...
      return unless available_decisions.count.positive?
      ...
      attempts = 0
      while !player_input_is_valid && (attempts < max_attempts)
        player_input = Readline.readline('❞ >', true)
        attempts += 1
        player_input_is_valid = event.player_input_valid?(player_input)

        unless player_input_is_valid
    ...
    DIRS_TO_LOAD = %w[config extensions models].freeze
